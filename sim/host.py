...
thread, and the main thread runs the viewer when `HUDATHON_VIEWER=1` (else it just serves).
...
class SimHost:
    ...
    async def start(self, timeout: float = 120.0) -> None:
        ...
        child_env = {**os.environ, "HUDATHON_SIM_SERVE": self.serve, "HUDATHON_SIM_PORT": str(self.port)}
...
async def _serve_robot(port: int) -> None:
    ...
    from sim.franka_bridge import HudathonFrankaBridge

    bridge = HudathonFrankaBridge(port=0)  # ephemeral WS; the env reads it back via url()
    await bridge.start()
...
def main() -> None:
    serve = os.environ["HUDATHON_SIM_SERVE"]  # "mcp" | "robot", handed off by SimHost
    port = int(os.environ["HUDATHON_SIM_PORT"])
    ...
    if os.environ.get("HUDATHON_VIEWER") == "1":
